class InpaintSnowNonEmptyPatchAug(torch.nn.Module):
    ...
    def forward(self, datacube, seglabel):
        ...
        if (self.p>eps): 
            seglabel_inv = seglabel.numpy().copy()
            seglabel_inv[seglabel_inv==0] = 2
            seglabel_inv[seglabel_inv!=2] = 0
            seglabel_inv = seglabel_inv//2
            ...
            snow_mask = snow_mask*seglabel_inv # for avoiding overlapping
            ...
            new_mask = torch.tensor(snow_mask)+seglabel#torch.tensor(cloud_mask)+torch.tensor(shadow_mask)
